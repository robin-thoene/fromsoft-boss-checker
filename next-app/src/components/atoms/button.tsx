...
export default function Button(props: IButtonProps): ReactElement {
    return (
        <button
            className={`flex flex-row items-center ${props.fullWidth ? 'w-full' : 'w-max'} p-3 ${props.outlined ? 'outline outline-1' : ''} ${props.isDangerous ? 'text-red-500 outline-red-500' : ''} rounded-lg h-max`}
