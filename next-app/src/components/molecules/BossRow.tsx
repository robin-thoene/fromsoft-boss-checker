 ...
 */
export function BossRow(props: IBossRowProps) {
  ...
  const rowContent = useMemo(
    () => (
      <>
        <div
          className={`flex items-center ${props.felledBossIds.includes(props.boss.id) ? "line-through" : ""}`}
        >
          <Checkbox
            isChecked={props.felledBossIds.includes(props.boss.id)}
            disabled={!props.isInitialized}
            onChange={() => toggleFelledState(props.boss.id)}
          />
          ...
          <Button
            icon={
              <EllipsisVerticalIcon
                className={`h-4 w-4 ${props.markedBossIds.includes(props.boss.id) ? "text-red-600" : ""}`}
              />
            }
  ...
  return (
    <div className="flex w-full items-center">
      <div className="w-full">
        <div className="flex sm:hidden flex-row w-full gap-4 justify-between">
          {rowContent}
        </div>
        <div
          className="cursor-pointer hidden sm:flex flex-row w-full gap-4 justify-between"
          onClick={() => toggleFelledState(props.boss.id)}
        >
          {rowContent}
        </div>
      </div>
