 ...
 * @param {IContextActionsProps} props - The component properties.
 * @returns {ReactElement} The rendered context actions menu.
 */
export default function ContextActions(props: IContextActionsProps): ReactElement {
    /**
    ...
    return (
        <>
            ...
            {props.fromSoftwareGame === FromSoftwareGame.EldenRing && (props.boss as IEldenRingBoss).wikiMapReference && (
                <Button
                    icon={<MapPinIcon className="h-5 w-5" />}
                    onClick={(e) => {
                        e.stopPropagation();
                        e.preventDefault();
                        window.open((props.boss as IEldenRingBoss).wikiMapReference, '_blank');
                    }}
                />
            )}
            <Button
                icon={<FlagIcon className={`h-5 w-5 ${props.markedBossIds.includes(props.boss.id) ? 'text-red-600' : ''}`} />}
                onClick={(e) => {
                    e.stopPropagation();
                    e.preventDefault();
