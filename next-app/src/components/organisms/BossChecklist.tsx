 ...
 */
export function BossChecklist(props: IBossChecklistProps) {
  ...
  const bossCounter = useMemo(() => {
    ...
    if (props.regions) {
      return props.regions.reduce(
        (acc, region) => acc + region.bosses.length,
        0,
      );
    }
    return 0;
  ...
  const [isInitializedClientSide, setIsInitializedClientSide] =
  ...
  useEffect(() => {
    ...
    if (props.regions && props.bosses) {
      throw new Error(
        'The properties "regions" and "bosses" are mutually exclusive. Please use only one of them.',
      );
    }
    // Load the users stored progress from the local storage.
    const storedFelledBossIdsString = localStorage.getItem(
      props.localStorageFelledBossesKey,
    );
    ...
    const storedMarkedBossIdsString = localStorage.getItem(
      props.localStorageMarkedBossesKey,
    );
    ...
    if (props.localStorageCollapsedRegionsKey && props.regions) {
      const storedMarkedRegionIdsString = localStorage.getItem(
        props.localStorageCollapsedRegionsKey,
      );
  ...
  const RegionCollapseButton = useCallback(
    ({ regionId }: { regionId: number }) => (
      <Button
        icon={
          collapsedRegions.includes(regionId) ? (
  ...
  return (
    <div className="flex flex-1 flex-col overflow-auto p-10 max-w-full overflow-x-hidden">
        ...
        <div className="flex flex-col gap-10">
          {props.regions?.map((region) => (
            <div
              ...
              key={`region-${region.id}-${region.name}`}
            >
                ...
                <div className="hidden sm:flex items-center">
                  <div className="w-full">
                    <Button
                      ...
                      onClick={() => linkToRegion(region.name)}
                    />
                  </div>
              ...
              <div
                className={`flex flex-col overflow-hidden ${collapsedRegions.includes(region.id) ? "h-0" : "h-full py-3"}`}
              >
      ...
      <div className="flex w-full mt-16">
        <Button
          outlined
          text={props.dic["gameProgress_reset_button"]}
          fullWidth
      ...
      <GameProgressIndicator
        totalAmount={bossCounter}
        felledCount={felledBossIds.length}
      />
    </div>
  );
}
